...
export type CropWizardPanelProps = {
  ...
  onExtract: (boxes: BBox[], removeOriginal: boolean) => void;
  onCancel?: () => void;
  /** When true, show the "Remove original" checkbox (default true) */
...
function useCropState(
  ...
  useEffect(() => {
    ...
    if (useAISuggestions && aiBoxes && aiBoxes.length) {
      setRects(aiBoxes.map((b) => ({ ...b, id: uid(), source: "ai" as const })));
    } else {
      setRects([]);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [active, aiBoxes, useAISuggestions]);
  ...
  const remove = (id: string) => setRects((prev) => prev.filter((r) => r.id !== id));

  return { imgRef, rects, setRects, drawing, onDown, onMove, onUp, remove };
}

// ── Headless panel (no Dialog wrapper) ───────────────────────────────────────
...
export function CropWizardPanel({
  ...
  const { imgRef, rects, setRects, drawing, onDown, onMove, onUp, remove } =
    useCropState(true, aiBoxes, useAISuggestions);
              ...
              previews.map((p) => (
                ...
                >
                  <span className="flex items-center gap-1">
                    {p.source === "ai" && <Sparkles className="h-3 w-3 text-primary" />}
...
// ── Dialog wrapper (used by old flow) ────────────────────────────────────────
